...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    player_score_one , opponent_score_one , opponent_score_ten = player_score%10 , opponent_score%10 ,opponent_score//10%10
    if abs(player_score_one - opponent_score_one) == opponent_score_ten:
        flag = True
    else:
        flag = False
    return flag
...
def make_averaged(original_function, trials_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    def new_func(*args):
        #run original function some number of times
        #return average of function calls
        res = 0
        i=0
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    scores = [averaged_dice(i+1, dice) for i in range(10)]
    num_rolls = scores.index(max(scores)) + 1
    return num_rolls
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    f_b = free_bacon(opponent_score)
    if is_swap(score+f_b , opponent_score):
        if opponent_score <= score + f_b:
            return num_rolls
        else:
            return 0
    else:
